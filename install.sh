...
# @@Created          :  Wednesday, Jul 05, 2023 19:59 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for aria2
...
HUB_IMAGE_URL="casjaysdevdocker/aria2"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [aria2.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-aria2}}"
...
if [ -n "$ARIA2_USERNAME" ]; then
  CONTAINER_USER_NAME="$ARIA2_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${ARIA2_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$ARIA2_PASSWORD" ]; then
  CONTAINER_USER_PASS="$ARIA2_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${ARIA2_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
